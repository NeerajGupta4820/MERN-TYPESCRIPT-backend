...
export const getDashboardStats = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) stats = JSON.parse(myCache.get(key) as string);
    ...
    stats = {/*categoryCount,*/changePercent,count,chart: {order: orderMonthCounts,revenue: orderMonthyRevenue,},
      userRatio,latestTransaction: modifiedLatestTransaction,
    };
...
export const getPieCharts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) charts = JSON.parse(myCache.get(key) as string);
    ...
    const stockAvailablity = {inStock: productsCount - outOfStock,outOfStock,};
...
export const getLineCharts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) charts = JSON.parse(myCache.get(key) as string);
    ...
    charts = {users: usersCounts,products: productCounts,discount,revenue,};
    myCache.set(key, JSON.stringify(charts));
  }
